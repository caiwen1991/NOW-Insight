...
- **Hero trend chart** (`src/components/now/TrendChart.tsx`) — a LIVE price line with selectable
  ...
  - **1D price line is color-coded (intentional exception to the neutral-verdict rule).** On the
    1D range only, the actual price LINE (and the hover dot) is drawn **green (`--pos`) when the
    current price is up vs. the previous close, red (`--neg`) when down** (`baselineUp =
    data.change >= 0`, passed into `TrendPlot` → `lineColor`). The previous close equals the price at
    12am (market is closed overnight). The dashed "prev close" benchmark line stays its neutral
    `--hero-ink` color; only the price line takes red/green, and other ranges keep the brand-green
    accent line. This is a deliberate, scoped deviation from the otherwise no-red/green up-down rule.
